...
\begin{enumerate}[label=\alph*)]
  \item Executar o \textit{testbench} no ModelSim/Questa, capturar as formas de onda dos principais sinais e registrar eventuais mensagens de \textit{assert}.
  \item Compilar o projeto no Quartus Prime, exportar o diagrama RTL e preencher a Tabela~\ref{tab:resource-usage} com os recursos utilizados e a frequência máxima.
  \item Inserir no texto comentários sintéticos sobre os resultados obtidos (tempo de convergência observado, recursos, limitações).
\end{enumerate}

Após essas etapas, recomenda-se uma revisão final para inserir as figuras geradas e atualizar a discussão dos resultados na Figura~\ref{fig:simulation-waveforms} e na Tabela~\ref{tab:resource-usage}, encerrando a avaliação.
