...
A partir do algoritmo, identificaram-se seis estados para o controle: \code{s_IDLE}, \code{s_LOAD}, \code{s_COMPARE}, \code{s_SUB_X}, \code{s_SUB_Y} e \code{s_DONE}. A Figura~\ref{fig:fsm-states} mostra o diagrama desenhado manualmente na Seção de síntese, a Figura~\ref{fig:fsm-states-quartus} confirma essa mesma FSM exportada pelo Quartus Prime. A Tabela~\ref{tab:fsm-transicoes} detalha as transições e saídas relevantes.

\begin{figure}[H]
  \centering
  \includeplaceholder{media/states.jpeg}{Reservar espaço para diagrama de estados da FSM}
  \caption{Diagrama inicial da máquina de estados elaborado manualmente.}
...
Os códigos foram escritos seguindo o guia de estilo da avaliação: palavras reservadas em minúsculas, sinais em maiúsculas com prefixos (\code{i_}, \code{o_}, \code{w_}, \code{r_}), tabulação de dois espaços.
...
\begin{figure}[H]
  \centering
  \includeplaceholder{media/states_quartus.jpg}{Reservar espaço para o diagrama de estados exportado pelo Quartus}
  \caption{FSM sintetizada pelo Quartus Prime, confirmando a implementação VHDL.}
  \label{fig:fsm-states-quartus}
\end{figure}
